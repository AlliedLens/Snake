...
-on being eaten by snake, snake should have size incremeted by one

create two-player functionality(LONG TERM)
- get a second snake on the board on the same time as the first snake, with its own seperate controls
- get the snake to kill the other snake upon collision
...
BCG_COLOR = (0,0,0)
TITLE_COLOR = (0, 255, 65)
TITLE_BCG_COLOR = (214, 137, 16)
MENUBUTTON_COLOR = (0, 255, 65)
...
class Snake():
	def __init__(self, centerX, centerY, bodyColor, headColor, length):
	...
	def isHeadColliding(self, pellet): # checks if the head collides with the pellet, snake, or border
...
def game():
	...
	if game_start:
		...
		player1 = Snake(CENTER_X, CENTER_Y, BODY1COLOR, HEAD1COLOR, 1)
		player2 = Snake(CENTER_X, CENTER_Y, BODY2COLOR, HEAD2COLOR, 1)
		...
		while game_start:
			...
			for event in pygame.event.get():
				...
				if event.type == pygame.KEYDOWN:
					...
					if event.unicode == "x":
						print(pellet1.centerX, pellet1.centerY)
			...
			if player1.isHeadColliding(pellet1): # takes you to back to the boot screen if any collision takes place
				game_start = False
				menu_start = True
				return True	
